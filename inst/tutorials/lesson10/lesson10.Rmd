...
1. It would be nice to see the source code for a function (what's going on behind the scenes when you use a function), you just specify the name without any parentheses or arguments. Try typing just `qqnormsim` into your console and hit enter. What happens?

```{r, eval = F}
qqnormsim
```

You should be seeing the source code for the function. Pick one line that has some code you've seen before and make an educated guess at what it is doing. 
...
2. Does the normal probability plot for the calories from fat in Dairy Queen items generated in the pre-lab look similar to the plots created for the simulated data?  That is, do the plots provide evidence that the calories from fat are nearly normal? Explain.
